  ...
  const renderChanges = (field: any, index: number) => {
    return (
      <TableRow key={index} className={styles.tableRow}>
        ...
        <TableCell>
          {field === 'payment_source' ? selectedEvent.object[field].brand : selectedEvent.object[field] || '-'}
        </TableCell>
      </TableRow>
    );
  };
    ...
    >
      ...
      <div className={styles.content}>
        {selectedEvent.type !== 'customer.created' ? (
            ...
            <Table>
              ...
              <TableBody>
                {selectedEvent.object &&
                  Object.keys(selectedEvent.object).map((field: string, index: number) => {
          ...
          <Typography className={styles.customerCreated}>Customer created</Typography>
        )}
      </div>
    </Modal>
  );
