...
const OnboardingInfo: FC<IOnboardingInfo> = ({ courier, handleUpdateOnboard }) => {
  return (
    <div>
      <SummaryItem title='Welcome Package' value={courier.onboarded ? 'Yes' : 'Mark as sent'} onClick={!courier.onboarded ? handleUpdateOnboard : undefined } />
      ...
      <SummaryItem title='Set Billing Account?' value={courier.dwolla && courier.dwolla.bankAccountType ? 'Yes' : 'No'} />
    </div>
  );
};
